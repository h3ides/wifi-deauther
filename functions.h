...
#include "Arduino.h"
#include <LittleFS.h>
#include <ESP8266WiFi.h>   // <-- added to use Arduino WiFi API where available

extern "C" {
  #include "user_interface.h"

  // Declare SDK prototypes so code compiles on cores that still expose these functions.
  // If the SDK does not provide them at link time you may get a linker error;
  // in that case the Arduino API fallback (WiFi.*) will be used (where possible).
  // Keep these declarations here to silence "was not declared" compile errors.
...
void setWifiChannel(uint8_t ch, bool force) {
    if (((ch != wifi_channel) || force) && (ch < 15)) {
        ...
        // Try to use Arduino WiFi API if it exposes a channel setter. If not,
        // fall back to the SDK wifi_set_channel that many older cores provide.
        //
        // Note: some Arduino ESP8266 core versions removed the SDK wrappers,
        // which is why the SDK prototype is declared above. If the SDK function
        // is not present at link time you may see a linker error — in that case,
        // you can remove the SDK fallback and use only the Arduino APIs.
        //
        // The explicit SDK call is preserved because the deauther project relies
        // on low-level channel control in some environments.
        #if defined(WiFi) && defined(ESP8266)
            // If the environment provides a helper to set the channel via WiFi
            // object, prefer that (some cores add WiFi.setChannel).
            #ifdef WIFI_SET_CHANNEL_DEFINED
                WiFi.setChannel(ch);
            #else
                // Fallback to direct SDK call (declared above)
                wifi_set_channel(wifi_channel);
            #endif
        #else
            // Last resort (if not ESP8266 or no WiFi available): try SDK call
            wifi_set_channel(wifi_channel);
        #endif
    }
}
...
void setOutputPower(float dBm) {
    ...
    // Use Arduino WiFi API if available (preferred). It accepts float dBm on ESP8266.
    #if defined(ESP8266)
        WiFi.setOutputPower(dBm);
    #else
        uint8_t val = (dBm * 4.0f);
        system_phy_set_max_tpw(val); // fallback to SDK function
    #endif
}
